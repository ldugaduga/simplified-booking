...
Laravel 12 + Inertia/Vue app. PHP deps via Composer, JS deps via npm (`package-lock.json`).

- Dev (server, queue, logs, Vite): `composer run dev` (http://localhost:8000)
- Dev server only: `php artisan serve`
- Build: `npm run build`
- Test: `php artisan test` (PHPUnit; needs `npm run build` first for Inertia page tests)
- Lint (PHP): `./vendor/bin/pint`
- Lint (frontend): `npm run lint` (ESLint with `--fix`)
- Format (frontend): `npm run format` (check only: `npm run format:check`)
- Fresh local database: `php artisan migrate:fresh --seed` (deletes local data)

No combined `Verify` command is defined yet; run `/ci` to set one up.

Browser testing is opt-in. Run `/tests browser` to add a browser harness and
document its exact command as `Browser tests`.
